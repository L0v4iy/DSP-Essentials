...
namespace Unity.Audio
{
    ...
    public unsafe partial struct DSPGraph
    {
        ...
        private OwnedAtomicQueue<GrowableBuffer<IntPtr>> m_Commands;
        ...
        private OwnedAtomicQueue<DSPNode> m_NodesPendingDisposal;

        internal OwnedAtomicQueue<EventHandlerDescription> MainThreadCallbacks;
        ...
        DSPGraph(SoundFormat outputFormat, int outputChannels, int dspBufferSize, int sampleRate)
        {
            ...
            *m_LastReadLength = 0;
            ...
            m_Commands = OwnedAtomicQueue<GrowableBuffer<IntPtr>>.Create();
            MainThreadCallbacks = OwnedAtomicQueue<EventHandlerDescription>.Create();
            m_NodesPendingDisposal = OwnedAtomicQueue<DSPNode>.Create();
            ...
            m_HandleAllocator.Acquire(out Handle.Node * node);
            ...
            where T : unmanaged, IDSPCommand
        {
            ...
            m_CommandAllocator.Acquire(out var command);
            return (T*)command;
        }
        ...
        {
            m_Commands.Enqueue(ref commands);
        }
        ...
        {
            while (m_Commands.TryDequeue(out GrowableBuffer<IntPtr> commandBuffer))
            {
        ...
        internal void ScheduleDSPNodeDisposal(Handle nodeHandle)
        {
            ...
            m_NodesPendingDisposal.Enqueue(ref node);
        }
        ...
        internal void ExecuteUpdateJob(Handle node, void* jobStructMemory, void* updateReflectionData, void* jobReflectionData, Handle requestHandle = default)
        {
            ...
            var callbackDescription = new EventHandlerDescription { Handler = description.Callback };
            if (description.Callback.IsAllocated)
                MainThreadCallbacks.Enqueue(ref callbackDescription);
        }
